...
)
{
    ...
    const half T_min = __float2half_rn(0.0001f);
...
#pragma unroll
    for(int k = 0; k < 8; ++k)
    {
        ...
        bool valid_x = __hgt(exponents.x, threshold_low) && __hlt(exponents.x, threshold_high);
        bool valid_y = __hgt(exponents.y, threshold_low) && __hlt(exponents.y, threshold_high);
